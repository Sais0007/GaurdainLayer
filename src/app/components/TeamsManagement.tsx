...
export default function TeamsManagement() {
  ...
  const [formAllowedModels, setFormAllowedModels] = useState<string[]>(["gpt-4o"]);
  const [formMaxBudget, setFormMaxBudget] = useState("2000");
  ...
  const filteredTeams = useMemo(() => {
    ...
    return result;
  }, [teams, searchQuery, filterOrg, filterStatus, sortField, sortDirection]);
  ...
  const handleCreateTeamSubmit = () => {
    ...
    const newTeam: TeamItem = {
      ...
      accessGroupsCount: 1,
      currentSpend: 0,
      maxBudget: parseFloat(formMaxBudget) || 2000,
      tpmLimit: 250000,
      rpmLimit: 2500,
      budgetDuration: "Monthly",
      softBudgetPercent: 80,
      ...
      membersList: [{ id: "m-100", name: "John Doe", email: "john.doe@company.com", userId: "usr-904128", role: "Team Admin", models: formAllowedModels, budget: parseFloat(formMaxBudget) || 2000, currentSpend: 0, status: "Active", lastActive: "Just now", addedDate: "Just now" }],
      keysList: [],
    ...
    toast.success(`Team "${formName}" created successfully!`);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto space-y-6">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-2xl max-w-4xl w-full flex flex-col max-h-[90vh] overflow-hidden">
            <div className="px-6 py-4 border-b flex items-center justify-between flex-shrink-0 bg-white dark:bg-neutral-900">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-primary-50 rounded-xl text-primary-600"><Users className="w-5 h-5" /></div>
                <div>
                  <h3 className="text-base font-bold text-neutral-900 dark:text-white">Create Team</h3>
                  <p className="text-xs text-neutral-500 mt-0.5">Create a new team and configure its default access, models, and budget.</p>
                </div>
              </div>
              <button onClick={() => setShowCreateModal(false)}><X className="w-5 h-5 text-neutral-400" /></button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-5 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full h-10 px-3 border rounded-lg" placeholder="Enter Team Name" />
                </div>
                <div>
                  <label className="block font-semibold mb-1">Organization *</label>
                  <select value={formOrg} onChange={(e) => setFormOrg(e.target.value)} className="w-full h-10 px-3 border rounded-lg">
                    <option value="HB Enterprise">HB Enterprise</option>
                    <option value="Spine CloudIQ">Spine CloudIQ</option>
                  </select>
                </div>
            ...
            <div className="px-6 py-4 border-t flex justify-between bg-neutral-50 dark:bg-neutral-900 flex-shrink-0">
              <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 text-xs font-semibold text-neutral-600">Cancel</button>
              <PrimaryButton onClick={handleCreateTeamSubmit}>Create Team</PrimaryButton>
            </div>
          </div>
        </div>
      )}
