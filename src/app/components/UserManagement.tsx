...
const ALL_COLUMNS: ColumnDef[] = [
  ...
  { key: 'spendUsd', label: 'Spend / Budget' },
  { key: 'role', label: 'Role' },
...
export default function UserManagement() {
  ...
  const [viewModeState, setViewModeState] = useState<'list' | 'detail' | 'create' | 'edit'>('list');
  const [selectedLegacyUser, setSelectedLegacyUser] = useState<any | null>(null);
    ...
    return (
      <div className="p-6 bg-transparent dark:bg-neutral-950">
        <UserForm
          ...
          onSave={(newUser) => {
            setUsersList((prev) => [newUser, ...prev]);
            toast.success(`User "${newUser.userAlias || newUser.email}" invited successfully!`);
            setViewModeState('list');
          }}
        />
      </div>
    ...
    return (
      <UserEdit
        ...
        onSave={(updatedUser) => {
          ...
          toast.success(`User "${updatedUser.userAlias || updatedUser.email}" updated successfully!`);
          setViewModeState('list');
        }}
      />
    );
  ...
  return (
    <div className="p-6 bg-transparent dark:bg-neutral-950 space-y-6">
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-neutral-50/80 dark:bg-neutral-900/80 border-b border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400 font-semibold select-none">
                    ...
                    {/* Combined Spend / Budget Header */}
                    {(visibleColumns.spendUsd || visibleColumns.budgetUsd) && (
                      <th onClick={() => handleSort('spendUsd')} className="py-3 px-4 cursor-pointer hover:text-neutral-900 dark:hover:text-white transition-colors">
                        <div className="flex items-center gap-1">
                          ...
                          {sortField === 'spendUsd' || sortField === 'budgetUsd' ? (sortDirection === 'asc' ? <ArrowUp className="w-3 h-3 text-primary-600" /> : <ArrowDown className="w-3 h-3 text-primary-600" />) : <ArrowUpDown className="w-3 h-3 text-neutral-400 opacity-50" />}
                        </div>
                      </th>
                    )}
                  ...
                  {isLoading ? (
                    Array.from({ length: 5 }).map((_, idx) => (
                      <tr key={idx} className="animate-pulse">
                        ...
                        {(visibleColumns.spendUsd || visibleColumns.budgetUsd) && <td className="py-4 px-4"><div className="w-24 h-7 bg-neutral-200 dark:bg-neutral-800 rounded" /></td>}
                            ...
                            <td className="py-3 px-4 font-mono">
                              <div className="flex flex-col">
                                ...
                                <span className="text-[11px] text-neutral-400 font-normal mt-0.5">
                                  of {user.budgetUsd === null ? "Unlimited" : `$${user.budgetUsd.toFixed(2)} ${user.budgetDuration || "Monthly"}`}
                                </span>
                              </div>
                            </td>
                          )}
      ...
      {showInviteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn overflow-y-auto">
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-2xl max-w-4xl w-full flex flex-col max-h-[90vh] overflow-y-auto my-auto p-6">
            <UserForm
              mode="create"
