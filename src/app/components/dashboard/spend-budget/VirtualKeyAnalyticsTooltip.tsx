...
import { VirtualKeyHealthStatus } from "./VirtualKeyStatusChip";

export interface VirtualKeyDetailData {
  keyAlias: string;
  assignedTeam: string;
  assignedUser: string;
  spend: number;
...
interface VirtualKeyAnalyticsTooltipProps {
  data: VirtualKeyDetailData;
  targetRect: DOMRect | null;
}

export const VirtualKeyAnalyticsTooltip: React.FC<VirtualKeyAnalyticsTooltipProps> = ({
  data,
  targetRect,
}) => {
  ...
  // Calculate smart fixed coordinates to prevent clipping and ensure visibility above all elements
  const viewportWidth = window.innerWidth;
  const viewportHeight = window.innerHeight;

  // Vertical position: place smoothly relative to hovered row, flip up if near viewport bottom
  let top = targetRect.top + 4;
  if (top + TOOLTIP_HEIGHT > viewportHeight - 10) {
    top = Math.max(10, targetRect.bottom - TOOLTIP_HEIGHT - 4);
  }

  // Horizontal position: place on right side if space available, bounded within viewport
  let left = targetRect.right - 260;
  ...
  return createPortal(
    <div
      ...
      className="!bg-[#1C1F2E] text-white p-4 rounded-[14px] shadow-[0_20px_50px_rgba(0,0,0,0.7)] border border-neutral-700/90 text-xs space-y-2 w-72 pointer-events-none select-none animate-fadeIn"
    >
      {/* Tooltip Header */}
      <div className="font-bold border-b border-neutral-700/80 pb-2 flex justify-between items-center text-emerald-400 font-sans text-sm">
        <span className="font-mono text-xs truncate max-w-[170px] text-white">{data.keyAlias}</span>
      ...
      {/* Aligned 2-Column Details Grid */}
      <div className="space-y-1.5 font-mono text-[11px] pt-1">
        <div className="flex justify-between items-center text-[10px]">
          <span className="text-neutral-400">Assigned Team:</span>
          <span className="text-white font-semibold font-sans">{data.assignedTeam}</span>
        </div>
        <div className="flex justify-between items-center text-[10px]">
          <span className="text-neutral-400">Assigned User:</span>
          <span className="text-white font-semibold font-sans">{data.assignedUser}</span>
        </div>
        ...
        <div className="pt-1.5 border-t border-neutral-700/60 flex justify-between items-center">
          <span className="text-neutral-400">Monthly Requests:</span>
        ...
        <div className="flex justify-between items-center">
          <span className="text-neutral-400">Avg Cost / Req:</span>
        ...
        <div className="flex justify-between items-center text-[9px] text-neutral-400 pt-0.5">
          <span>Last Activity:</span>
          <span className="text-neutral-300 font-sans">{data.lastActivity}</span>
        </div>
      </div>
    </div>,
    document.body
