...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 sm:p-6 bg-neutral-50 dark:bg-neutral-950 min-h-screen space-y-6">
      <div className="max-w-[100%] mx-auto space-y-6">
        ...
        <PageHeader
          ...
          action="list"
          title="Dashboard"
          subtitle="Organization Cost & Consumption Analytics"
        >
          <div className="flex flex-wrap items-center gap-2.5">

            {/* Dev State Manager */}
            <div className="flex items-center gap-1 bg-neutral-100 dark:bg-neutral-800/90 p-1 rounded-lg border border-neutral-200 dark:border-neutral-700">
              <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-400 px-1.5">State:</span>
              {(['normal', 'loading', 'empty', 'error'] as const).map((s) => (
                <button
                  key={s}
                  onClick={() => setState(s)}
                  className={cn(
                    "px-2.5 py-0.5 text-[11px] font-semibold rounded-md transition-all capitalize",
                    state === s
                      ? "bg-white dark:bg-neutral-900 shadow-xs text-primary-600 dark:text-primary-400"
                      : "text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-300"
                  )}
            ...
            <div className="relative">
              <select
                ...
                className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-xs font-semibold rounded-lg pl-3 pr-8 py-2 text-neutral-800 dark:text-neutral-200 outline-none focus:ring-1 focus:ring-primary-500 cursor-pointer shadow-xs appearance-none"
              >
              ...
              <Calendar className="w-3.5 h-3.5 absolute right-2.5 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none" />
            </div>
            ...
            <Button
              ...
              className="h-9 text-xs border-neutral-200 dark:border-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-900"
            >
            ...
            <Button
              size="sm"
              className="h-9 text-xs bg-primary-600 hover:bg-primary-700 text-white dark:bg-primary-500 dark:hover:bg-primary-600 shadow-xs"
            >
        ...
        {/* ================= TAB 1: DASHBOARD (COST ANALYTICS) ================= */}
        {activeTab === 'dashboard' ? (
          <div className="space-y-6">

            {/* SECTION 1: 5 PROPER KPI CARDS */}
                ...
                return (
                  <div key={kpi.id}>
                    {state === 'loading' ? (
                      ...
                      <Card className="p-4 border-dashed border-neutral-200 dark:border-neutral-800">
                        <div className="text-xs font-semibold text-neutral-600 dark:text-neutral-400">{kpi.title}</div>
                        <div className="text-xs text-neutral-400 mt-2">No usage recorded</div>
                      </Card>
                    ) : state === 'error' ? (
                      <Card className="p-4 border-rose-200 dark:border-rose-950/40 bg-rose-50/20">
                      ...
                      <Card className="p-4 border border-neutral-200 dark:border-neutral-800/80 bg-white dark:bg-neutral-900 hover:shadow-md transition-all duration-200 group cursor-pointer flex flex-col justify-between min-h-[148px]">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 truncate">
                            {kpi.title}
                          </span>
                          <div className={cn("p-1.5 rounded-lg border flex-shrink-0 transition-transform group-hover:scale-105", kpi.bg, kpi.color)}>
                        ...
                        <div className="my-2 flex items-end justify-between gap-2">
                          <div className="space-y-1">
                            <div className="text-2xl font-extrabold text-neutral-900 dark:text-white tracking-tight leading-none">
                              {kpi.value}
                            </div>
                            <div className="flex items-center gap-1 text-[11px] whitespace-nowrap">
                              <span className={cn(
                                ...
                                kpi.trend === 'up' 
                                  ? (kpi.id === 'failed_requests' ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400")
                        ...
                        <div className="pt-2 border-t border-neutral-100 dark:border-neutral-800 text-[10px] text-neutral-400 font-medium truncate">
                          {kpi.helperText}
                        </div>
                      </Card>
                    )}
            ...
            {/* SECTION 2: SPEND TREND (HERO VISUALIZATION) */}
            <Card className="border border-neutral-200/80 dark:border-neutral-800 shadow-sm">
              <CardHeader className="pb-2 border-b border-neutral-100 dark:border-neutral-800/80">
                <div>
              ...
              <CardContent className="pt-6">
                {state === 'loading' ? (
                  <div className="h-[280px] w-full flex items-end justify-between gap-3 px-2">
                    {[40, 65, 30, 85, 50, 75, 60, 90].map((h, i) => (
                      <Skeleton key={i} className="w-full rounded-md" style={{ height: `${h}%` }} />
                    ))}
                  </div>
                ) : state === 'empty' ? (
                  <div className="h-[280px] flex flex-col items-center justify-center text-center">
                    <Inbox className="w-12 h-12 text-neutral-300 dark:text-neutral-700 mb-2 stroke-[1.2]" />
                    <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">No spend history recorded</p>
                    <p className="text-[11px] text-neutral-400 mt-1 max-w-[280px]">Run requests through virtual keys to start tracking spend trends.</p>
                  </div>
                ) : (
            ...
            {/* SECTION 3 & 4: TOP TEAMS & TOP MEMBERS BY SPEND */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <AnalyticsWidget
                ...
                chartView={
                  <div className="h-[260px] w-full pt-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={filteredTeams} layout="vertical" margin={{ top: 5, right: 20, left: 40, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(128,128,128,0.12)" />
                        <XAxis type="number" stroke="#888888" fontSize={11} tickFormatter={(v) => `$${v}`} />
                        <YAxis dataKey="name" type="category" stroke="#888888" fontSize={10} width={110} tickLine={false} />
                        <Tooltip formatter={(value: any) => [`$${value}`, 'Spend']} />
                        <Bar dataKey="spend" fill="#0284c7" radius={[0, 6, 6, 0]} barSize={20} />
                      </BarChart>
                    </ResponsiveContainer>
                  ...
                  <table className="w-full text-xs text-left text-neutral-700 dark:text-neutral-300">
                    <thead className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 bg-neutral-100/70 dark:bg-neutral-900/70 border-b border-neutral-200 dark:border-neutral-800">
                      <tr>
                        ...
                        <th className="py-2.5 px-3 cursor-pointer hover:text-neutral-900 dark:hover:text-white" onClick={() => setTeamsSort({ key: 'spend', dir: teamsSort.dir === 'asc' ? 'desc' : 'asc' })}>
                          <div className="flex items-center gap-1">
                            Spend (USD) <ArrowUpDown className="w-3 h-3" />
                          </div>
                        </th>
                    ...
                    <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                      {filteredTeams.map((team) => (
                        <tr key={team.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                          <td className="py-2.5 px-3 font-semibold text-neutral-900 dark:text-white">{team.name}</td>
                          <td className="py-2.5 px-3 font-bold text-primary-600 dark:text-primary-400">${team.spend.toFixed(2)}</td>
                          <td className="py-2.5 px-3 text-emerald-600 font-medium">{(team.requests - team.failedRequests).toLocaleString()}</td>
                          <td className="py-2.5 px-3 text-rose-500 font-medium">{team.failedRequests.toLocaleString()}</td>
                          <td className="py-2.5 px-3 font-medium text-violet-600 dark:text-violet-400">{team.tokens}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* SECTION 4: TOP MEMBERS BY SPEND */}
              <AnalyticsWidget
                ...
                chartView={
                  <div className="h-[260px] w-full pt-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={filteredMembers} layout="vertical" margin={{ top: 5, right: 20, left: 40, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(128,128,128,0.12)" />
                        <XAxis type="number" stroke="#888888" fontSize={11} tickFormatter={(v) => `$${v}`} />
                        <YAxis dataKey="name" type="category" stroke="#888888" fontSize={10} width={100} tickLine={false} />
                        <Tooltip formatter={(value: any) => [`$${value}`, 'Spend']} />
                        <Bar dataKey="spend" fill="#6366f1" radius={[0, 6, 6, 0]} barSize={20} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                }
                  ...
                  <table className="w-full text-xs text-left text-neutral-700 dark:text-neutral-300">
                    <thead className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 bg-neutral-100/70 dark:bg-neutral-900/70 border-b border-neutral-200 dark:border-neutral-800">
                      <tr>
                        ...
                        <th className="py-2.5 px-3 cursor-pointer hover:text-neutral-900 dark:hover:text-white" onClick={() => setMembersSort({ key: 'spend', dir: membersSort.dir === 'asc' ? 'desc' : 'asc' })}>
                          <div className="flex items-center gap-1">
                    ...
                    <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                      {filteredMembers.map((member) => (
                        <tr key={member.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                          <td className="py-2.5 px-3">
                            <div className="font-semibold text-neutral-900 dark:text-white">{member.name}</div>
                            <div className="text-[10px] text-neutral-400">{member.team}</div>
                          </td>
                          <td className="py-2.5 px-3 font-bold text-primary-600 dark:text-primary-400">${member.spend.toFixed(2)}</td>
                          <td className="py-2.5 px-3 font-medium">{member.requests.toLocaleString()}</td>
                          <td className="py-2.5 px-3 font-medium text-violet-600 dark:text-violet-400">{member.tokens}</td>
                          <td className="py-2.5 px-3 font-semibold text-emerald-600">{member.successRate}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {/* SECTION 5 & 6: TOP VIRTUAL KEYS & TOP MODELS */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">

              {/* SECTION 5: TOP VIRTUAL KEYS */}
              <AnalyticsWidget
                ...
                onViewModeChange={setVirtualKeysView}
                hasSearch={true}
                searchQuery={virtualKeysSearch}
                onSearchChange={setVirtualKeysSearch}
                state={state}
                chartView={
                  <div className="h-[260px] w-full pt-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={filteredVirtualKeys} layout="vertical" margin={{ top: 5, right: 20, left: 50, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(128,128,128,0.12)" />
                        <XAxis type="number" stroke="#888888" fontSize={11} tickFormatter={(v) => `$${v}`} />
                        <YAxis dataKey="alias" type="category" stroke="#888888" fontSize={10} width={120} tickLine={false} />
                        <Tooltip formatter={(value: any) => [`$${value}`, 'Spend']} />
                        <Bar dataKey="spend" fill="#10b981" radius={[0, 6, 6, 0]} barSize={20} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                }
                  ...
                  <table className="w-full text-xs text-left text-neutral-700 dark:text-neutral-300">
                    <thead className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 bg-neutral-100/70 dark:bg-neutral-900/70 border-b border-neutral-200 dark:border-neutral-800">
                      <tr>
                        ...
                        <th className="py-2.5 px-3 cursor-pointer hover:text-neutral-900 dark:hover:text-white" onClick={() => setVirtualKeysSort({ key: 'spend', dir: virtualKeysSort.dir === 'asc' ? 'desc' : 'asc' })}>
                          <div className="flex items-center gap-1">
                            Spend (USD) <ArrowUpDown className="w-3 h-3" />
                          </div>
                        </th>
                    ...
                    <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                      {filteredVirtualKeys.map((vk) => (
                        <tr key={vk.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                          <td className="py-2.5 px-3 font-semibold text-neutral-900 dark:text-white">{vk.alias}</td>
                          <td className="py-2.5 px-3 font-mono text-[11px] text-neutral-400">{vk.keyId}</td>
                          <td className="py-2.5 px-3 font-bold text-primary-600 dark:text-primary-400">${vk.spend.toFixed(2)}</td>
                          <td className="py-2.5 px-3 font-medium">{vk.requests.toLocaleString()}</td>
                          <td className="py-2.5 px-3 font-medium text-violet-600 dark:text-violet-400">{vk.tokens}</td>
                        </tr>
              ...
              <AnalyticsWidget
                ...
                chartView={
                  <div className="h-[260px] w-full pt-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={filteredModels} layout="vertical" margin={{ top: 5, right: 20, left: 40, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(128,128,128,0.12)" />
                        <XAxis type="number" stroke="#888888" fontSize={11} tickFormatter={(v) => `$${v}`} />
                        <YAxis dataKey="model" type="category" stroke="#888888" fontSize={10} width={110} tickLine={false} />
                        <Tooltip formatter={(value: any) => [`$${value}`, 'Spend']} />
                        <Bar dataKey="spend" fill="#8b5cf6" radius={[0, 6, 6, 0]} barSize={20} />
                      </BarChart>
                    </ResponsiveContainer>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs text-left text-neutral-700 dark:text-neutral-300">
                    <thead className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 bg-neutral-100/70 dark:bg-neutral-900/70 border-b border-neutral-200 dark:border-neutral-800">
                      <tr>
                        <th className="py-2.5 px-3">Model</th>
                        <th className="py-2.5 px-3 cursor-pointer hover:text-neutral-900 dark:hover:text-white" onClick={() => setModelsSort({ key: 'spend', dir: modelsSort.dir === 'asc' ? 'desc' : 'asc' })}>
                          <div className="flex items-center gap-1">
                            Spend <ArrowUpDown className="w-3 h-3" />
                          </div>
                        </th>
                    ...
                    <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                      {filteredModels.map((md) => (
                        <tr key={md.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                          <td className="py-2.5 px-3">
                            <div className="font-semibold text-neutral-900 dark:text-white font-mono text-[11px]">{md.model}</div>
                            <div className="text-[10px] text-neutral-400">{md.provider}</div>
                          </td>
                          <td className="py-2.5 px-3 font-bold text-primary-600 dark:text-primary-400">${md.spend.toFixed(2)}</td>
                          <td className="py-2.5 px-3 text-emerald-600 font-medium">{(md.requests - md.failedRequests).toLocaleString()}</td>
                          <td className="py-2.5 px-3 text-rose-500 font-medium">{md.failedRequests.toLocaleString()}</td>
                          <td className="py-2.5 px-3 font-medium text-violet-600 dark:text-violet-400">{md.tokens}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <Card className="border border-neutral-200/80 dark:border-neutral-800 shadow-sm">
              ...
              <CardContent className="pt-6">
                {state === 'loading' ? (
                  ...
                  <div className="py-10 flex flex-col items-center justify-center text-center">
                    <Inbox className="w-10 h-10 text-neutral-300 dark:text-neutral-700 mb-2 stroke-[1.5]" />
                    <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">No provider usage recorded</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
                    ...
                    <div className="lg:col-span-2 space-y-3">
                      {providerUsageData.map((provider) => (
                        <div key={provider.name} className="p-3 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200/60 dark:border-neutral-800 space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: provider.color }}></div>
                              <span className="font-bold text-xs text-neutral-900 dark:text-white">{provider.name}</span>
                              <Badge variant="outline" className="text-[10px] py-0 px-1.5 font-semibold">
                                {provider.percent}% of spend
                              </Badge>
                            </div>
                          ...
                          <div className="w-full bg-neutral-200 dark:bg-neutral-800 h-2 rounded-full overflow-hidden">
                            <div 
                              ...
                              style={{ width: `${provider.percent}%`, backgroundColor: provider.color }}
                            ></div>
                          </div>

                          <div className="flex justify-between items-center text-[11px] text-neutral-500 pt-0.5">
                            <span>Requests: <strong className="text-neutral-800 dark:text-neutral-200">{provider.requests.toLocaleString()}</strong></span>
                            <span>Tokens: <strong className="text-neutral-800 dark:text-neutral-200">{provider.tokens}</strong></span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          /* ================= TAB 2: MODEL ACTIVITY ================= */
          <div className="space-y-6">

            {/* SECTION 1: OVERALL USAGE CARDS (4 CARDS - Title Case Labels) */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {modelOverallKpis.map((kpi) => {
                const IconComponent = kpi.icon;
                return (
                  <div key={kpi.id}>
                    {state === 'loading' ? (
                      ...
                      <Card className="p-4 border border-neutral-200 dark:border-neutral-800/80 bg-white dark:bg-neutral-900 hover:shadow-md transition-all duration-200 group cursor-pointer flex flex-col justify-between min-h-[148px]">
                        ...
                        <div className="my-2 flex items-end justify-between gap-2">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-1 text-[11px] whitespace-nowrap">
                              <span className="font-bold flex items-center gap-0.5 text-emerald-600 dark:text-emerald-400">
            ...
            {/* SECTION 2: CHARTS ROW (2 EQUAL WIDTH CARDS) */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">

              {/* LEFT: TOTAL TOKENS OVER TIME */}
              <Card className="border border-neutral-200/80 dark:border-neutral-800 shadow-sm">
                <CardHeader className="pb-2 border-b border-neutral-100 dark:border-neutral-800/80">
                  <CardTitle className="text-sm font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                  ...
                  <CardDescription className="text-xs text-neutral-500">
                    Input vs Output Token distribution across all models
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-6">
                  {state === 'loading' ? (
                    ...
                    <div className="h-[260px] flex flex-col items-center justify-center text-center">
                      <Inbox className="w-10 h-10 text-neutral-300 dark:text-neutral-700 mb-2 stroke-[1.5]" />
                      <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">No token activity found</p>
                    </div>
                  ) : (
                    <div className="h-[260px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={modelOverviewTokensData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(128,128,128,0.12)" />
                          <XAxis dataKey="date" stroke="#888888" fontSize={11} tickLine={false} axisLine={false} />
                          ...
                          <Area type="monotone" dataKey="inputTokens" name="Input Tokens" stroke="#0284c7" fill="#0284c7" fillOpacity={0.25} />
                          <Area type="monotone" dataKey="outputTokens" name="Output Tokens" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.25} />
                          <Area type="monotone" dataKey="totalTokens" name="Total Tokens" stroke="#6366f1" fill="#6366f1" fillOpacity={0.15} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  )}
              ...
              {/* RIGHT: TOTAL REQUESTS OVER TIME */}
              <Card className="border border-neutral-200/80 dark:border-neutral-800 shadow-sm">
                <CardHeader className="pb-2 border-b border-neutral-100 dark:border-neutral-800/80">
                  <CardTitle className="text-sm font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                  ...
                  <CardDescription className="text-xs text-neutral-500">
                    Successful (HB Green) vs Failed (HB Red) API executions
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-6">
                  {state === 'loading' ? (
                    ...
                    <div className="h-[260px] flex flex-col items-center justify-center text-center">
                      <Inbox className="w-10 h-10 text-neutral-300 dark:text-neutral-700 mb-2 stroke-[1.5]" />
                      <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">No request activity found</p>
                    </div>
                  ) : (
                    <div className="h-[260px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={modelOverviewRequestsData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(128,128,128,0.12)" />
                          <XAxis dataKey="date" stroke="#888888" fontSize={11} tickLine={false} axisLine={false} />
                          <YAxis stroke="#888888" fontSize={11} tickLine={false} axisLine={false} />
                          <Tooltip />
                          <Legend iconSize={8} wrapperStyle={{ fontSize: '11px', paddingTop: '8px' }} />
                          <Area type="monotone" dataKey="successful" name="Successful Requests" stroke="#10b981" fill="#10b981" fillOpacity={0.25} />
                          <Area type="monotone" dataKey="failed" name="Failed Requests" stroke="#f43f5e" fill="#f43f5e" fillOpacity={0.25} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  )}
            ...
            {/* SECTION 3: MODEL-WISE COLLAPSIBLE ACCORDION CARDS */}
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-neutral-900 dark:text-white">
                  Deployed Model Analytics ({deployedModels.length})
                </h3>
                <span className="text-xs text-neutral-400">Click accordion to expand model breakdown</span>
              </div>

              {state === 'empty' ? (
                <Card className="p-10 text-center border-dashed border-neutral-300 dark:border-neutral-800">
                  <Inbox className="w-10 h-10 text-neutral-400 mx-auto mb-2 stroke-[1.2]" />
                  <h4 className="text-sm font-bold text-neutral-800 dark:text-neutral-200">No Model Activity Found</h4>
                  <p className="text-xs text-neutral-500 mt-1 max-w-xs mx-auto mb-4">No AI model activity logged for the selected time range.</p>
                ...
                deployedModels.map((model) => (
                  <ModelAccordionCard
                    ...
                    onToggle={() => setExpandedModelId(expandedModelId === model.id ? '' : model.id)}
                    state={state}
                  />
                ))
              )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="border border-neutral-200/80 dark:border-neutral-800 shadow-sm">
                <CardHeader className="pb-2 border-b border-neutral-100 dark:border-neutral-800/80">
                  ...
                  <CardDescription className="text-xs text-neutral-500">
                    Input vs Output Token distribution across all virtual keys
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-6">
                  {state === 'loading' ? (
              ...
              <Card className="border border-neutral-200/80 dark:border-neutral-800 shadow-sm">
                <CardHeader className="pb-2 border-b border-neutral-100 dark:border-neutral-800/80">
                  ...
                  <CardDescription className="text-xs text-neutral-500">
                    Successful (HB Green) vs Failed (HB Red) virtual key call executions
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-6">
                  {state === 'loading' ? (
            ...
            {/* SECTION 3: VIRTUAL KEY-WISE COLLAPSIBLE ACCORDION CARDS */}
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-neutral-900 dark:text-white">
                  Virtual Key Analytics ({virtualKeysList.length})
                </h3>
                <span className="text-xs text-neutral-400">Click accordion to expand virtual key breakdown</span>
              </div>

              {state === 'empty' ? (
                <Card className="p-10 text-center border-dashed border-neutral-300 dark:border-neutral-800">
                  <Inbox className="w-10 h-10 text-neutral-400 mx-auto mb-2 stroke-[1.2]" />
                  <h4 className="text-sm font-bold text-neutral-800 dark:text-neutral-200">No activity found for this Virtual Key</h4>
                  <p className="text-xs text-neutral-500 mt-1 max-w-xs mx-auto mb-4">Requests made using this key will appear here.</p>
                  <Button size="sm" onClick={handleRefresh} variant="outline" className="text-xs">
                    <RefreshCw className="w-3.5 h-3.5 mr-1.5" /> Refresh Data
                  </Button>
                </Card>
              ) : (
                virtualKeysList.map((vk) => (
                  <VirtualKeyAccordionCard
                    key={vk.id}
                    vk={vk}
                    isExpanded={expandedVkId === vk.id}
                    onToggle={() => setExpandedVkId(expandedVkId === vk.id ? '' : vk.id)}
                    state={state}
                  />
                ))
              )}
